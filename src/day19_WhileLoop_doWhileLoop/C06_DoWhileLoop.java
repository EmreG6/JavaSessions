...
public class C06_DoWhileLoop {
	...
	private static boolean ozelKarakterKontrol(String sifre) {
		...
		if (!okk) {
			System.out.println("Sifreniz en az 1 ozel karakter icermelidir");
	...
	private static boolean buyukHarfKontrol(String sifre) {
		...
		if (!bhk) {
			System.out.println("Sifreniz en az 1 buyuk harf icermelidir");
	...
	private static boolean kucukHarfKontrol(String sifre) {
		...
		for (int i = 0; i < sifre.length(); i++) {
			if (harfler.contains(sifre.substring(i, i + 1))) {
				khk = true;
				break; // break kullanmamaizin nedeni : soruda 1 tane kcuuk harf olmasi yeterli
						// biz kucuk harfi bulduktan sonra kodu durdurmazsak
						// sonradan karsimiza cikacak Buyuk harfler bizi yanýltýr
			}

		}
		if (!khk) {
			System.out.println("Sifreniz en az 1 kucuk harf icermelidir");
